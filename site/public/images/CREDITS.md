...
Every photographer name, profile and licence below was read from the linked
Unsplash photo page before the file was downloaded, and cross-checked against
the filename Unsplash's own download endpoint returned. The verification
evidence is recorded in `.superpowers/sdd/IMPLEMENTATION_PLAN/task-17-evidence.md`.

Two registers are deliberately maintained. **Mitte** is warm, low-lit and
dark-wooded; **Charlottenburg** is a pale Gründerzeit Altbau in daylight. The
chrome around them does not change — only the photography and the address do.
...
The `photography` item stays open. The practice must still approve or replace
all twelve. Nothing in `lib/open-facts.ts` was touched by this task.
